...
var generateMap = function() 
{
    genRandom();
    var tiles = getTiles();
    tiles = tiles.split("|").join("0").split("_").join("0");
    tiles = tiles.split(".").join("1").split("o").join("1");
    tiles = tiles.split(" ").join("1").split("-").join("1");
    tiles = tiles.split("").map(function (x) { return parseInt(x, 10);});
    
    const result = [];
    while(tiles.length) result.push(tiles.splice(0, MAP_WIDTH));
    for (var i = 0; i < result.length; ++i)
    {
        result[i][0] = 0;
        result[i][MAP_WIDTH - 1] = 0;
    }
